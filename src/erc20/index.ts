...
// 获取BNB余额
export const GetBalanceOfBNB = async (
  web3: Web3,
  account: string
): Promise<string> => {
...
export const VerifyTransferByHash = async (
  web3: Web3,
  hash: string
): Promise<{
  token: string;
  ...
  if (
    to &&
    decodeData.method === "transfer" &&
    decodeData.inputs &&
    decodeData.inputs.length === 2
  ) {
    const decimals = await GetTokenDecimals(web3, to);
    const receiveAddress = decodeData.inputs[0];
    const amount = Number(
      formatUnits(decodeData.inputs[1].toString(), decimals)
    );
